...
For essentially all purposes, including selfplay game generation and match play, we highly recommend using the latest `release/version` branch (for example `release/0.31.1`), which is equivalent to using the latest version tag.
...
```shell
git clone -b release/0.31.1 --recurse-submodules https://github.com/LeelaChessZero/lc0.git
```
...
git fetch --all
git branch --all
git checkout -t remotes/origin/release/0.31.1
```


If you prefer to download an archive, you need to also download and place the submodule:
 * Download the [.zip](https://api.github.com/repos/LeelaChessZero/lc0/zipball/release/0.31.1) file ([.tar.gz](https://api.github.com/repos/LeelaChessZero/lc0/tarball/release/0.31.1) archive is also available)
 * Extract
