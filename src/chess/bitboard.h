...
class Move {
 public:
  ...
  void SetFrom(BoardSquare from) { data_ = (data_ & ~FROM_MASK_) | (from.as_int() << 6); }
  void SetPromotion(Promotion promotion) { data_ = (data_ & ~PROMO_MASK_) | ((uint8_t)promotion << 12); }
  ...
  operator bool() const { return GetFrom().as_int() != 0 || GetTo().as_int() != 0; }
