...
// Square a1 is bit 0, square a8 is bit 7, square b1 is bit 8.
    ...
    public:
        ...
        BoardSquare GetFrom() const {
          return (BoardSquare)((data_ & FROM_MASK_) >> 6);
        }
        Promotion GetPromotion() const {
          return (Promotion)((data_ & PROMO_MASK_) >> 12);
        }
        ...
        void SetFrom(BoardSquare from) {
          data_ = (data_ & ~FROM_MASK_) | (from.as_int() << 6);
        }
        void SetPromotion(Promotion promotion) {
          data_ = (data_ & ~PROMO_MASK_) | ((uint8_t)promotion << 12);
        }
        ...
        operator bool() const {
          return GetFrom().as_int() != 0 || GetTo().as_int() != 0;
        }

        void Mirror() {
          BoardSquare tmpfrom = GetFrom();
          BoardSquare tmpto = GetTo();
          tmpfrom.Mirror();
          tmpto.Mirror();
          SetFrom(tmpfrom);
          SetTo(tmpto);
    ...
    private:
        ...
        enum MASKS_ : uint16_t {
            ...
            FROM_MASK_ = 0b0000111111000000,
            PROMO_MASK_ = 0b0111000000000000,
            CASTLE_MASK_ = 0b1000000000000000,
        };
    };
