...
void Node::FinalizeScoreUpdate(float v, float d, float m, int k, float p0, float a) {
  ...
  float invp = 1.0f / p;
  float a = n_ * 1.0f / (n_ + k);
  float b = k  * 1.0f / (n_ + k);
  wl_ = std::clamp(std::pow(a * std::pow(0.5f * wl_ + 0.5f, p) +
                            b * std::pow(0.5f * v   + 0.5f, p), invp) * 2 - 1,
                   -1.0f, 1.0f);
  d_ =  std::clamp(std::pow(a * std::pow(d_, p) +
                            b * std::pow(d,  p), invp), -1.0f, 1.0f);
  m_ += k * (m - m_) / (n_ + k);
