...
#if 0
#define HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS 1
#else
#define HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS 0
#undef hipEventWaitExternal
#undef hipEventRecordExternal
#endif

namespace lczero {
using namespace hip_backend;
...
template <typename DataType>
class HipNetworkComputation : public NetworkComputation {
 public:
  HipNetworkComputation(HipNetwork<DataType>* network, bool wdl,
                         bool moves_left);
  ~HipNetworkComputation();
 ...
 private:
  ...
  HipNetwork<DataType>* network_;
};

template <typename DataType>
class HipNetwork : public Network {
 public:
  HipNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    min_batch_size_ =
        options.GetOrDefault<int>("min_batch", std::min(1, max_batch_size_));
    ...
    ReportHIPErrors(hipGetDeviceProperties(&deviceProp, gpu_id_));
    ...
    // layout used by hip backend is nchw.
    ...
    if (!multi_stream_) {
      ...
      ReportHIPErrors(hipEventCreateWithFlags(&compute_ordering_event_,
    ...
    auto allocateCudaGraphs = [&] {
      ReportHIPErrors(hipSetDevice(gpu_id_));
      HipNetworkComputation<DataType> comp(this, wdl_, moves_left_);
  ...
  CudaGraphCapture<DataType> BeginCapture(InputsOutputs<DataType>& io) {
    if (!multi_stream_) {
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
  ...
  void UploadInputs(InputsOutputs<DataType>* io, int batchSize) {
    ...
    ReportHIPErrors(
        hipMemcpyAsync(io->input_masks_mem_gpu_, io->input_masks_mem_,
                        batchSize * kInputPlanes * sizeof(uint64_t),
                        hipMemcpyHostToDevice, upload_stream_));
    ReportHIPErrors(hipMemcpyAsync(
        io->input_val_mem_gpu_, io->input_val_mem_,
        batchSize * kInputPlanes * sizeof(io->input_val_mem_[0]),
        hipMemcpyHostToDevice, upload_stream_));
    ReportHIPErrors(hipEventRecord(io->upload_done_event_, upload_stream_));
    ReportHIPErrors(
        hipStreamWaitEvent(compute_stream_, io->upload_done_event_, 0));
  }
  ...
  void GraphLaunch(InputsOutputs<DataType>* io, int batchSize) {
    ...
    if (!multi_stream_) {
      ...
      ReportHIPErrors(
          hipEventRecord(io->download_done_event_, compute_stream_));
    } else {
      io->cuda_graphs_[batchSize - 1].Launch(io->exec_stream_);
      ReportHIPErrors(
          hipEventRecord(io->download_done_event_, io->exec_stream_));
    }
#endif
  }
  ...
  void forwardEval(InputsOutputs<DataType>* io, int batchSize,
    ...
    hipStream_t compute_stream, upload_stream, download_stream;
    ...
    if (multi_stream_ || HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS) {
      ReportHIPErrors(
          hipMemcpyAsync(io->input_masks_mem_gpu_, io->input_masks_mem_,
                          batchSize * kInputPlanes * sizeof(uint64_t),
      ...
      ReportHIPErrors(hipMemcpyAsync(
          ...
          batchSize * kInputPlanes * sizeof(io->input_val_mem_[0]),
          hipMemcpyHostToDevice, upload_stream));
      ReportHIPErrors(hipEventRecord(io->upload_done_event_, upload_stream));
      ReportHIPErrors(
          hipStreamWaitEvent(compute_stream, io->upload_done_event_, 0));
    }

    if (!multi_stream_) {
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
      ReportHIPErrors(
          hipStreamWaitEvent(compute_stream, compute_ordering_event_,
                              capture ? hipEventWaitExternal : 0));
#endif
    }
    ...
    if (numBlocks_ > 0) {
      ...
      network_[l++]->Eval(batchSize, skip_connection, tensor_mem[0], nullptr,
                          scratch_mem, scratch_size_, cublas,
    ...
    if (attn_body_) {
      network_[l++]->Eval(
          ...
          scratch_size_, cublas, compute_stream,
    ...
    ReportHIPErrors(hipEventRecord(io->policy_done_event_, compute_stream));
    ReportHIPErrors(
        hipStreamWaitEvent(download_stream, io->policy_done_event_, 0));
...
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
      ReportHIPErrors(
          hipEventRecordWithFlags(compute_ordering_event_, compute_stream,
                                   capture ? hipEventRecordExternal : 0));
#endif
    }
    ReportHIPErrors(hipEventRecord(io->value_done_event_, compute_stream));
    ReportHIPErrors(
        hipStreamWaitEvent(download_stream, io->value_done_event_, 0));
    ReportHIPErrors(hipMemcpyAsync(
    ...
    if (wdl_) {
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
      ReportHIPErrors(hipEventRecordWithFlags(
    ...
    if (moves_left_) {
      ...
      ReportHIPErrors(
          hipEventRecord(io->moves_left_done_event_, compute_stream));
      ReportHIPErrors(
          hipStreamWaitEvent(download_stream, io->moves_left_done_event_, 0));
      ReportHIPErrors(
          hipMemcpyAsync(io->op_moves_left_mem_, io->op_moves_left_mem_gpu_,
                          sizeof(io->op_moves_left_mem_[0]) * batchSize,
                          hipMemcpyDeviceToHost, download_stream));
    }
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
    ReportHIPErrors(
        hipEventRecordWithFlags(io->download_done_event_, download_stream,
                                 capture ? hipEventRecordExternal : 0));
#else
    if (!capture) {
      ReportHIPErrors(
          hipEventRecord(io->download_done_event_, download_stream));
    }
#endif
  }
...
#if !HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
    ReportHIPErrors(hipEventSynchronize(io->download_done_event_));
#endif
    if (wdl_) {
#if HIP_GRAPH_SUPPORTS_EXTERNAL_EVENTS
      ReportHIPErrors(hipEventSynchronize(io->wdl_download_done_event_));
#endif
  ...
  ~HipNetwork() {
    ...
    if (!multi_stream_) {
      ...
      ReportHIPErrors(hipEventDestroy(compute_ordering_event_));
    }
  }
  ...
  std::unique_ptr<NetworkComputation> NewComputation() override {
    ...
    int device = -1;
    ReportHIPErrors(hipGetDevice(&device));
    if (device != gpu_id_) {
      ReportHIPErrors(hipSetDevice(gpu_id_));
    }
 ...
 private:
  ...
  void showInfo() const {
    ...
    switch (ret) {
      case hipErrorNotInitialized:
        throw Exception("HIP driver and/or runtime could not be initialized");
      case hipErrorInsufficientDriver:
        throw Exception("No HIP driver, or one older than the HIP library");
      case hipErrorNoDevice:
        throw Exception("No HIP-capable devices detected");
    }
    const int major_mul = 10000000;
    const int minor_mul = 100000;
    int major = version / major_mul;
    int minor = (version - major * major_mul) / minor_mul;
    int pl = version - major * major_mul - minor * minor_mul;
    CERR << "HIP Runtime version: " << major << "." << minor << "." << pl;
    if (version != HIP_VERSION) {
      major = HIP_VERSION / major_mul;
      minor = (HIP_VERSION - major * major_mul) / minor_mul;
      pl = HIP_VERSION - major * major_mul - minor * minor_mul;
      // After cuda 11, newer version with same major is OK.
      if ((major != version / major_mul) || version < HIP_VERSION) {
        CERR << "WARNING: HIP Runtime version mismatch, was compiled with "
                "version "
             << major << "." << minor << "." << pl;
      }
    }
    ReportHIPErrors(hipDriverGetVersion(&version));
    major = version / major_mul;
    minor = (version - major * major_mul) / minor_mul;
    pl = version - major * major_mul - minor * minor_mul;
    CERR << "Latest version of HIP supported by the driver: " << major << "."
         << minor << "." << pl;
    if (version < HIP_VERSION) {
  ...
  void showDeviceInfo(const hipDeviceProp_t& deviceProp,
    ...
    int clockRatekHz;
    hipError_t err =
        hipDeviceGetAttribute(&clockRatekHz, hipDeviceAttributeClockRate, deviceId);
    if (err != hipSuccess) {
      CERR << "Error getting clock rate: " << hipGetErrorString(err);
...
template <typename DataType>
HipNetworkComputation<DataType>::HipNetworkComputation(
    HipNetwork<DataType>* network, bool wdl, bool moves_left)
...
template <typename DataType>
void HipNetworkComputation<DataType>::CaptureGraph(
    std::unique_lock<std::mutex>&& lock) {
  if (!network_->GetGraphCaptureEnabled()) return;
  if (!CudaGraphCapture<DataType>::EnsureEnoughFreeMemory()) {
    static std::once_flag flag;
    std::call_once(flag, []() {
      CERR << "WARNING: Not enough GPU memory to capture HIP graphs.";
    });
    return;
  }
...
template <typename DataType>
void HipNetworkComputation<DataType>::ComputeBlocking() {
...
template <typename DataType>
std::unique_ptr<Network> MakeHipNetwork(const std::optional<WeightsFile>& w,
