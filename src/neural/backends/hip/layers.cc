...
void SELayer<float>::Eval(int N, float* output, const float* input,
                          ...
                          size_t scratch_size, hipblasHandle_t cublas,
  ...
  ReportHIPBLASErrors(hipblasSgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N,
                                  numFc1Out_, N, C, &alpha, w1_, C, op2, C,
                                  &beta, op1, numFc1Out_));
  addVectors(op1, b1_, op1, numFc1Out_ * N, numFc1Out_, numFc1Out_ * N, act_,
             stream);

  // 3. Second fully connected layer.
  ReportHIPBLASErrors(hipblasSgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, 2 * C, N,
...
void SELayer<half>::Eval(int N, half* output, const half* input,
                         ...
                         hipblasHandle_t cublas, hipStream_t stream, half***) {
  bool se_done = false;
  if (kUseFusedSELayer && nhwc_) {
  ...
  if (!se_done) {
    ...
    // 2. First fully connected layer.
    hipblasHalf alpha = 0x3C00;
    hipblasHalf beta = 0;
    ReportHIPBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N,
                                    numFc1Out_, N, C, &alpha, (hipblasHalf*)w1_,
                                    C, (hipblasHalf*)op2, C, &beta,
                                    (hipblasHalf*)op1, numFc1Out_));
    addVectors(op1, b1_, op1, numFc1Out_ * N, numFc1Out_, numFc1Out_ * N, act_,
               stream);

    // 3. Second fully connected layer.
    ReportHIPBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, 2 * C,
                                    N, numFc1Out_, &alpha, (hipblasHalf*)w2_,
                                    numFc1Out_, (hipblasHalf*)op1, numFc1Out_,
                                    &beta, (hipblasHalf*)op2, 2 * C));
    addVectors(op2, b2_, op2, 2 * C * N, 2 * C, 2 * C * N, ACTIVATION_NONE,
               stream);
...
void FCLayer<half>::Eval(int N, half* output_tensor, const half* input_tensor,
  ...
  hipblasHalf beta = 0;
  ReportHIPBLASErrors(hipblasHgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N,
                                  num_outputs, N, num_inputs, &alpha, (hipblasHalf*)weights_,
                                  num_inputs, (hipblasHalf*)input_tensor, num_inputs, &beta,
...
void FCLayer<float>::Eval(int N, float* output_tensor,
  ...
  float alpha = 1.0f, beta = 0.0f;
  ReportHIPBLASErrors(hipblasSgemm(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N,
                                  num_outputs, N, num_inputs, &alpha, weights_,
                                  num_inputs, input_tensor, num_inputs, &beta,
...
void BaseLayer<half>::cublasRowMajorMatrixMul(const half* A, const half* B,
  ...
  ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
      cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &halfOne, B, HIP_R_16F, N,
      N * K, A, HIP_R_16F, K, K * M, &halfZero, Out, HIP_R_16F, N, N * M,
      batchSize, HIPBLAS_COMPUTE_16F, HIPBLAS_GEMM_DEFAULT));
}
...
void BaseLayer<float>::cublasRowMajorMatrixMul(const float* A, const float* B,
  ...
  if (use_gemm_ex_)
    ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
        cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &floatOne, B, HIP_R_32F, N,
        N * K, A, HIP_R_32F, K, K * M, &floatZero, Out, HIP_R_32F, N, N * M,
        batchSize, HIPBLAS_COMPUTE_32F, HIPBLAS_GEMM_DEFAULT));
  else
    // Much slower on RTX 2060.. why? Maybe a cublas bug :-/
    ReportHIPBLASErrors(hipblasSgemmStridedBatched(
        cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &floatOne, B, N, N * K, A,
        K, K * M, &floatZero, Out, N, N * M, batchSize));
}

template <typename DataType>
void FusedWinogradConvSELayer<DataType>::Eval(
    int N, DataType* output, const DataType* input, const DataType* input2,
    void* scratch, size_t scratch_size, hipblasHandle_t cublas,
    hipStream_t stream, DataType***) {
...
void Conv1Layer<half>::cublasSpecialMatrixMul(const half* A, const half* B,
  ...
  ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
      cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &halfOne, B, HIP_R_16F, N,
      N * K, A, HIP_R_16F, K, 0, &halfZero, Out, HIP_R_16F, N, N * M, batchSize,
...
void Conv1Layer<float>::cublasSpecialMatrixMul(const float* A, const float* B,
  ...
  if (use_gemm_ex_)
    ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
        cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &floatOne, B, HIP_R_32F, N,
        N * K, A, HIP_R_32F, K, 0, &floatZero, Out, HIP_R_32F, N, N * M,
        batchSize, HIPBLAS_COMPUTE_32F, HIPBLAS_GEMM_DEFAULT));
  else
    // Much slower on RTX 2060.. why? Maybe a cublas bug :-/
    ReportHIPBLASErrors(hipblasSgemmStridedBatched(
        cublas, HIPBLAS_OP_N, HIPBLAS_OP_N, N, M, K, &floatOne, B, N, N * K, A,
        K, 0, &floatZero, Out, N, N * M, batchSize));
}
...
static void cublasXgemm(hipblasHandle_t handle, hipblasOperation_t transa,
  ...
  if (fp16) {
    ...
    ReportHIPBLASErrors(hipblasHgemm(
        handle, transa, transb, m, n, k, &alpha_h, (const hipblasHalf*)A,
        lda, (const hipblasHalf*)B, ldb, &beta_h, (hipblasHalf*)C, ldc));
  } else {
    ReportHIPBLASErrors(hipblasSgemm(handle, transa, transb, m, n, k, &alpha,
...
static void cublasXGemmStridedBatched(
    ...
    hipblasOperation_t transb, int m, int n, int k, float alpha, const void* A,
    int lda, long long int strideA, const void* B, int ldb,
    long long int strideB, float beta, void* C, int ldc, long long int strideC,
    int batchCount, bool use_gemm_ex) {
  const bool fp16 = std::is_same<half, DataType>::value;
  if (fp16) {
    unsigned short alpha_h = FP32toFP16(alpha);
    unsigned short beta_h = FP32toFP16(beta);
    ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
        handle, transa, transb, m, n, k, &alpha_h, A, HIP_R_16F, lda, strideA,
        B, HIP_R_16F, ldb, strideB, &beta_h, C, HIP_R_16F, ldc, strideC,
        batchCount, HIPBLAS_COMPUTE_16F, HIPBLAS_GEMM_DEFAULT));
  } else {
    if (use_gemm_ex) {
      ReportHIPBLASErrors(hipblasGemmStridedBatchedEx(
          handle, transa, transb, m, n, k, &alpha, A, HIP_R_32F, lda, strideA,
          B, HIP_R_32F, ldb, strideB, &beta, C, HIP_R_32F, ldc, strideC,
...
template <typename DataType>
static void cublasXGemmBatched(hipblasHandle_t handle,
  ...
  if (fp16) {
    unsigned short alpha_h = FP32toFP16(alpha);
    unsigned short beta_h = FP32toFP16(beta);
    ReportHIPBLASErrors(hipblasHgemmBatched(
        handle, transa, transb, m, n, k, &alpha_h, (hipblasHalf**)A, lda,
        (hipblasHalf**)B, ldb, &beta_h, (hipblasHalf**)C, ldc, batchCount));
  } else {
    ReportHIPBLASErrors(hipblasSgemmBatched(
...
void EncoderBlock<DataType>::Eval(int N, DataType* in_out_tensor,
                                  ...
                                  DataType* buffer2, hipblasHandle_t cublas,
                                  hipStream_t stream,
  ...
  if (has_smolgen_) {
...

    {
      ...
      const int batch = N;
      cublasXgemm<DataType>(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, num_outputs,
  ...
  */
  ...
  // matmul_qk = tf.matmul(q, k, transpose_b=True)
  {
    if (*offset_pointers == nullptr) {
      ...
      ReportHIPErrors(hipStreamIsCapturing(stream, &capture));
      assert(capture != hipStreamCaptureStatus::hipStreamCaptureStatusActive &&
...
void AttentionPolicyHead<DataType>::Eval(
    int N, DataType* output, const DataType* input, const DataType* input2,
    void* scratch, size_t scratch_size, hipblasHandle_t cublas,
    hipStream_t stream, DataType*** offset_pointers) {
  ...
  DataType* pol_embedding = input2_tensor;
  {
    ...
    const int batch = N * 64;
    cublasXgemm<DataType>(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, num_outputs,
                          batch, num_inputs, 1.0f, (const DataType*)ip_pol_w_,
...
void ValueHead<DataType>::Eval(int N, DataType* output, const DataType* input,
  ...
  DataType* buffer = (DataType*)input2;
  {
    ...
    if (attention_body_) {
      ...
      conv_->Eval(N, buffer, input, nullptr, scratch, scratch_size, 
                  cublas, stream);
    }
  }

  {
    ...
    cublasXgemm<DataType>(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, num_outputs,
                          batch, num_inputs, 1.0f, (const DataType*)ip1_val_w_,
...

  {
    ...
    cublasXgemm<DataType>(cublas, HIPBLAS_OP_T, HIPBLAS_OP_N, num_outputs,
                          batch, num_inputs, 1.0f, (const DataType*)ip2_val_w_,
                          num_inputs, (DataType*)scratch, num_inputs, 0.0f,
...
void CudnnError(miopenStatus_t status, const char* file, const int& line) {
  if (status != miopenStatusSuccess) {
    char message[128];
    sprintf(message, "CUDNN error: %s (%s:%d) ", miopenGetErrorString(status),
            file, line);
    CERR << message;
...
void CudaError(hipError_t status, const char* file, const int& line) {
  if (status != hipSuccess) {
    char message[128];
    sprintf(message, "CUDA error: %s (%s:%d) ", hipGetErrorString(status), file,
            line);
    CERR << message;
    throw Exception(message);
  }
