...
// miopenHandle_t is already defined in rocm_common.h
...
class BaseLayer {
 public:
  ...
  virtual void Eval(int N, DataType* output, const DataType* input,
                    ...
                    hipStream_t stream, DataType*** = nullptr) = 0;
 ...
 protected:
  ...
  void cublasRowMajorMatrixMul(const DataType* A, const DataType* B,
                               ...
                               int batchSize, rocblas_handle cublas);
};
...
class ConvLayer : public BaseLayer<DataType> {
 ...
 public:
  ...
  void Eval(int N, DataType* output, const DataType* input,
            const DataType* input2, void* scratch, size_t scratch_size,
            miopenHandle_t cudnn, rocblas_handle cublas, hipStream_t stream,
 ...
 private:
  ...
  miopenTensorDescriptor_t in_tensor_desc_;
  miopenTensorDescriptor_t out_tensor_desc_;
  miopenActivationDescriptor_t activation_;

  // MIOpen solution-based API
  uint64_t solution_id_ = 0;
  size_t solution_workspace_size_ = 0;
  bool solution_queried_ = false;
...
class PolicyMapLayer : public BaseLayer<DataType> {
 ...
 public:
  ...
  // FP16-only: Eval with FP32 output (converts during policy map)
  // Avoids separate FP16→FP32 conversion pass
  void EvalFp32Output(int N, float* output, const DataType* input,
                      hipStream_t stream);
...
class EncoderBlock {
 public:
  ...
  void Eval(int N, DataType* inpop, DataType* scratch0, DataType* scratch1,
            DataType* scratch2, rocblas_handle cublas, hipStream_t stream,
