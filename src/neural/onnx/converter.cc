...
std::string Converter::MakeEncoderLayer(
  ...
  if (ffn_activation == ACTIVATION_RELU_2 &&
    ...
    std::transform(ffn_dense2_w.begin(), ffn_dense2_w.end(),
                   ffn_dense2_w.begin(),
                   [shift](float f) { return f / shift; });
    std::transform(ffn_dense2_b.begin(), ffn_dense2_b.end(),
                   ffn_dense2_b.begin(),
                   [shift](float f) { return shift * f; });
  }
  auto ffn_in = MakeLayerNorm(builder, flow, name + "/ln1",
                              *GetWeghtsConverter(ln1_gammas, {embedding_size}),
                              *GetWeghtsConverter(ln1_betas, {embedding_size}));
