...
namespace lczero {
  ...
  namespace {
    ...
    public:
      ...
      CheckComputation(std::unique_ptr<NetworkComputation> refComp, std::unique_ptr<NetworkComputation> checkComp):
      ...
      int GetBatchSize() const override {
        return refComp_->GetBatchSize();
      }
      
      float GetQVal(int sample) const override {
        return refComp_->GetQVal(sample);
      }
    ...
    private:
      ...
      static bool IsAlmostEqual(float a, float b) {
        ...
        return abs(a-b) <= std::max(RELATIVE_TOLERANCE*std::max(abs(a), abs(b)), ABSOLUTE_TOLERANCE);
      }
    ...
    public:
      ...
      std::unique_ptr<NetworkComputation> NewComputation() override {
        ...
        if (check) {
          ...
          std::unique_ptr<NetworkComputation> checkComp=checkNet_->NewComputation();
          return std::make_unique<CheckComputation>(std::move(refComp), std::move(checkComp));
        }
