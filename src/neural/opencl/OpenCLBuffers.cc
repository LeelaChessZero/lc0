...
void OpenCLBuffers::forward(const std::vector<net_t>& input,
  ...
  m_commandqueue.enqueueWriteBuffer(m_inBuffer, CL_FALSE, 0, inSize, input.data());
  ...
  for (auto iter = cbegin(m_layers); iter != cend(m_layers); iter++) {
    ...
    if (layer.is_input_convolution) {
      ...
      convolve3(layer.channels, layer.outputs, m_inBuffer, m_inBuffer, m_VBuffer,
                m_MBuffer, conv_weights, nullptr, bn_weights, skip_in_trans,
      ...
      convolve3(layer.channels, layer.outputs, m_inBuffer, m_inBuffer2, m_VBuffer,
                m_MBuffer, conv1_weights, nullptr, bn1_weights, skip_in_trans,
      ...
      convolve3(layer.channels, layer.outputs, m_inBuffer2, m_inBuffer, m_VBuffer,
                m_MBuffer, conv2_weights, &m_inBuffer, bn2_weights, true,
                skip_next_in_trans, true, batch_size);
      skip_in_trans = skip_next_in_trans;
    } else {
      ...
      convolve1(layer.channels, layer.outputs, m_inBuffer, m_inBuffer2, m_VBuffer,
  ...
  m_commandqueue.enqueueMapBuffer(m_pinnedOutBuffer_pol, CL_FALSE,
                                  CL_MAP_READ, 0, batch_size * finalSize_pol);
  auto pinnedOutBufferHost_val =
  m_commandqueue.enqueueMapBuffer(m_pinnedOutBuffer_val, CL_FALSE,
...
void OpenCLBuffers::convolve3(int channels, int outputs, cl::Buffer& bufferIn,
  ...
  if (!skip_in_transform) {
    try {
      ...
      m_commandqueue.enqueueNDRangeKernel(m_in_transform_kernel, cl::NullRange,
  ...
  try {
    ...
    m_commandqueue.enqueueNDRangeKernel(m_sgemm_kernel, cl::NullRange, size_sgemm,
                                        local_sgemm);
  } catch (const cl::Error& e) {
    CERR << "Error in convolve3: " << e.what() << ": " << e.err()
    << std::endl;
  ...
  try {
    if (fuse_in_transform) {
      ...
      m_commandqueue.enqueueNDRangeKernel(m_out_transform_bn_in_kernel, cl::NullRange,
      ...
      m_commandqueue.enqueueNDRangeKernel(m_out_transform_bn_kernel, cl::NullRange,
                                          cl::NDRange(outputs, wgs, batch_size));
    }
  } catch (const cl::Error& e) {
    CERR << "Error in convolve3: " << e.what() << ": " << e.err()
...
void OpenCLBuffers::convolve1(int channels, int outputs,
  ...
  try {
    ...
    CERR << "Error in convolve1: " << e.what() << ": " << e.err()
    << std::endl;
    throw;
  }
  ...
  try {
    ...
    m_commandqueue.enqueueNDRangeKernel(m_merge_kernel, cl::NullRange,
