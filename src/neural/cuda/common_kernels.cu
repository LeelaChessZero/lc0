...
void LayerNorm(int N, int C, T* output, const T* input, const T* bias,
  ...
  // process 4 or 8 elements per thread to achieve close to peak memory bandwidth
  if (C > 16384) throw Exception("unsupported filter size");
  const int EL_PER_THREAD = (C <= 4096) ? 4 : (C <= 8192 && fp16) ? 8 : 16;

  if (C % EL_PER_THREAD != 0) throw Exception("unsupported filter size");

  dim3 blockDim, gridDim;
  blockDim.x = 32;
  blockDim.y = DivUp(C / EL_PER_THREAD, 32);
  blockDim.z =
  ...
  if (EL_PER_THREAD == 8)
    layer_norm_kernel_8_el_per_thread<<<gridDim, blockDim, 0, stream>>>(
        N, C, (half*)output, (const half*)input, (const half*)bias,
        (const half*)skip, (const half*)gammas, (const half*)betas, ep, alpha,
        act);
  else if (EL_PER_THREAD == 16)
    layer_norm_kernel_slow<T><<<gridDim, blockDim, 0, stream>>>(
