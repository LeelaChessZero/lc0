  ...
  CudaNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    // Input.
    {
      auto inputConv = std::make_unique<FusedWinogradConvSELayer<DataType>>(
          nullptr, kNumFilters, 8, 8, kNumInputPlanes, !mish_net, true, false,
          false, mish_net, 0, use_gemm_ex, use_res_block_winograd_fuse_opt_);
    ...
    for (int block = 0; block < numBlocks_; block++) {
      ...
      if (use_res_block_winograd_fuse_opt_) {
        ...
        auto conv1 = std::make_unique<FusedWinogradConvSELayer<DataType>>(
            getLastLayer(), kNumFilters, 8, 8, kNumFilters, !mish_net, true,
            false, false, mish_net, 0, use_gemm_ex);
        ...
        auto conv2 = std::make_unique<FusedWinogradConvSELayer<DataType>>(
            getLastLayer(), kNumFilters, 8, 8, kNumFilters, !mish_net, true,
            true, has_se, mish_net, se_k, use_gemm_ex);
    ...
    if (conv_policy_) {
      auto conv1 = std::make_unique<FusedWinogradConvSELayer<DataType>>(
          resi_last_, kNumFilters, 8, 8, kNumFilters, !mish_net, true, false,
      ...
      auto convPol = std::make_unique<Conv1Layer<DataType>>(
          ...
          !mish_net, true, mish_net, use_gemm_ex);
    ...
    if (moves_left_) {
      ...
      auto FCMov1 = std::make_unique<FCLayer<DataType>>(
          getLastLayer(), weights.ip1_mov_b.size(), 1, 1, !mish_net, true,
          false, false, mish_net);
      FCMov1->LoadWeights(&weights.ip1_mov_w[0], &weights.ip1_mov_b[0],
                          scratch_mem_);
      network_.emplace_back(std::move(FCMov1));

      // Last relu on moves_left_ is relu even if mish_net.
  ...
  std::unique_ptr<InputsOutputs> GetInputsOutputs() {
    ...
    if (free_inputs_outputs_.empty()) {
      return std::make_unique<InputsOutputs>(max_batch_size_, wdl_, moves_left_,
                                             tensor_mem_size_, scratch_size_);
    } else {
