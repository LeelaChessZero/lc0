  ...
  CudaNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    // Input.
    {
      auto inputConv = std::make_unique<FusedWinogradConvSELayer<DataType>>(
          nullptr, kNumFilters, 8, 8, kNumInputPlanes, !mish_net, true, false, false, mish_net,
    ...
    if (conv_policy_) {
      ...
      auto convPol = std::make_unique<Conv1Layer<DataType>>(
          resi_last_, weights.policy.biases.size(), 8, 8, kNumFilters, !mish_net,
          true, mish_net, use_gemm_ex);
    ...
    // Value head.
    {
      ...
      auto FCVal1 = std::make_unique<FCLayer<DataType>>(
          getLastLayer(), weights.ip1_val_b.size(), 1, 1, !mish_net, true, false, false, mish_net);
    ...
    if (moves_left_) {
      ...
      auto FCMov1 = std::make_unique<FCLayer<DataType>>(
          getLastLayer(), weights.ip1_mov_b.size(), 1, 1, !mish_net, true, false, false, mish_net);
