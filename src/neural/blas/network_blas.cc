    ...
    if (attn_policy_) {
      ...
      for (auto layer : weights_.pol_encoder) {
        ...
        FullyConnectedLayer<use_eigen>::Forward1D(
            ...
            head_buffer.data(), layer.mha.q_w.data(),
            layer.mha.q_b.data(), NONE, head_buffer2.data());
        // K
        FullyConnectedLayer<use_eigen>::Forward1D(
            batch_size * kSquares, embedding_size, layer.mha.k_b.size(),
            head_buffer.data(), layer.mha.k_w.data(),
            layer.mha.k_b.data(), NONE, head_buffer3.data());
        // V
        FullyConnectedLayer<use_eigen>::Forward1D(
            batch_size * kSquares, embedding_size, layer.mha.v_b.size(),
            head_buffer.data(), layer.mha.v_w.data(),
            layer.mha.v_b.data(), NONE, head_buffer4.data());
          ...
          for (auto j = size_t{0}; j < kSquares; j++) {
            for (auto head = size_t{0}; head < heads; head++) {          
              ...
              std::copy(
                ...
                head_buffer2.begin() + batchStart + j * d_model + head * depth
              );
            }
          }
        }
