...
void LayerNorm2DWithSkipConnection(const size_t batch_size, const size_t channels,
                                   float* data, const float* skip, const float* gammas,
  ...
  for (size_t i = 0; i < batch_size; i++) {
    ...
    for (size_t c = 0; c < channels; ++c) {
      data[i * channels + c] = betas[c] + gammas[c]
        * (data[i * channels + c] - mean) / std::sqrt(var + epsilon);
    }
  ...
  for (auto batch = size_t{0}; batch < batch_size; batch++) {
    ...
    if (use_eigen) {
      ...
      cblas_sgemm(CblasRowMajor, transpose_a ? CblasTrans : CblasNoTrans,
                  ...
                  M, N, K, scaling, A, transpose_a ? M : K,
                  B, transpose_b ? K : N, 0.0f, C, N);
#else
