...
class CudnnNetwork : public Network {
 public:
  CudnnNetwork(const WeightsFile& file, const OptionsDict& options)
    ...
    /*
    DPCT1093:97: The "gpu_id_" device may be not the one intended for use.
    ...
    */
    ...
    if (fp16) {
      ...
      /*
      DPCT1005:98: The SYCL device version is different from CUDA Compute
      Compatibility. You may need to rewrite this code.
      */
      if ((deviceProp.get_major_version() == 6 &&
           deviceProp.get_minor_version() != 1) ||
          /*
          DPCT1005:99: The SYCL device version is different from CUDA Compute
          Compatibility. You may need to rewrite this code.
          */
        ...
        // nhwc_ remains false.
      /*
      DPCT1005:100: The SYCL device version is different from CUDA Compute
      Compatibility. You may need to rewrite this code.
      */
    ...
    if (hasTensorCores)
      /*
      DPCT1027:101: The call to cublasSetMathMode was replaced with 0 because
      this call is redundant in SYCL.
      */
      ReportCUBLASErrors(0);  // Deprecated on CUDA 11.0 and later
    else if (fp16)
      /*
      DPCT1027:102: The call to cublasSetMathMode was replaced with 0 because
      this call is redundant in SYCL.
      */
    ...
    if (fp16) {
      int cuda_version;
      /*
      DPCT1043:103: The version-related API is different in SYCL. An initial
      code was generated, but you need to adjust it.
      */
      cuda_version = dpct::get_current_device().get_major_version();
      if (!hasTensorCores)
        use_custom_winograd_ = false;
      else if (kNumFilters >= 256 &&
               /*
               DPCT1005:104: The SYCL device version is different from CUDA
               Compute Compatibility. You may need to rewrite this code.
               */
               !(deviceProp.get_major_version() == 7 &&
                 deviceProp.get_minor_version() == 5 && cuda_version < 11000))
    ...
    if (residual_weight_size > 0.6 * deviceProp.get_global_mem_size()) {
    ...
    if (!custom_winograd_override && use_custom_winograd_ &&
        transformed_residual_weight_size >
            0.5 * deviceProp.get_global_mem_size()) {
    ...
    if (use_custom_winograd_ && transformed_residual_weight_size >
                                    0.4 * deviceProp.get_global_mem_size()) {
    ...
    /*
    DPCT1005:105: The SYCL device version is different from CUDA Compute
    Compatibility. You may need to rewrite this code.
    */
    ...
    ReportCUDAErrors(
        DPCT_CHECK_ERROR(scratch_mem_ = (void*)sycl::malloc_device(
                             scratch_size_, dpct::get_default_queue())));
#ifdef DEBUG_RAW_NPS
    ...
    for (auto& mem : tensor_mem_) {
      ReportCUDAErrors(DPCT_CHECK_ERROR(
          mem = (typename std::remove_reference<decltype(mem)>::type)
              sycl::malloc_device(maxSize, dpct::get_default_queue())));
      ReportCUDAErrors(DPCT_CHECK_ERROR(
          dpct::get_default_queue().memset(mem, 0, maxSize).wait()));
    }
  ...
  void forwardEval(InputsOutputs* io, int batchSize) {
    ...
    dpct::queue_ptr stream = &dpct::get_default_queue();  // default stream
    ...
    if (attn_policy_) {
      ...
      if (fp16) {
        ...
        copyTypeConverted(opPol, (sycl::half*)(tensor_mem_[1]),
      ...
      if (fp16) {
        network_[l++]->Eval(batchSize, tensor_mem_[0], tensor_mem_[1], nullptr,
                            scratch_mem_, scratch_size_, cudnn_, cublas_,
                            stream);  // policy map layer
        copyTypeConverted(opPol, (sycl::half*)(tensor_mem_[0]),
                          batchSize * kNumOutputPolicy,
                          stream);  // POLICY output
      } else {
    ...
    ReportCUDAErrors(DPCT_CHECK_ERROR(dpct::get_default_queue().memcpy(
    ...
    if (fp16) {
      ...
      network_[l++]->Eval(batchSize, tensor_mem_[0], tensor_mem_[1], nullptr,
                          scratch_mem_, scratch_size_, cudnn_, cublas_,
                          stream);  // value FC2
      copyTypeConverted(opVal, (sycl::half*)(tensor_mem_[0]),
    ...
    if (moves_left_) {
      ...
      if (fp16) {
        ...
        network_[l++]->Eval(batchSize, tensor_mem_[0], tensor_mem_[1], nullptr,
                            scratch_mem_, scratch_size_, cudnn_, cublas_,
                            stream);
        copyTypeConverted(opMov, (sycl::half*)(tensor_mem_[0]), batchSize,
                          stream);
      } else {
  ...
  ~CudnnNetwork() {
    ...
    if (scratch_mem_) ReportCUDAErrors(
        DPCT_CHECK_ERROR(sycl::free(scratch_mem_, dpct::get_default_queue())));
  ...
  std::unique_ptr<NetworkComputation> NewComputation() override {
    ...
    /*
    DPCT1093:106: The "gpu_id_" device may be not the one intended for use.
    Adjust the selected device if needed.
    */
    ReportCUDAErrors(DPCT_CHECK_ERROR(dpct::select_device(gpu_id_)));
 ...
 private:
  ...
  void showInfo() const try {
    int version;
    /*
    DPCT1043:108: The version-related API is different in SYCL. An initial code
    ...
    */
    ...
    /*
    DPCT1043:107: The version-related API is different in SYCL. An initial code
    was generated, but you need to adjust it.
    */
    version = dpct::get_current_device().get_major_version();
    major = version / 1000;
  ...
  void showDeviceInfo(const dpct::device_info& deviceProp) const {
    ...
    /*
    DPCT1005:109: The SYCL device version is different from CUDA Compute
    Compatibility. You may need to rewrite this code.
    */
    if (version < 7301 &&
        (deviceProp.get_major_version() > 7 ||
         /*
         DPCT1005:110: The SYCL device version is different from CUDA Compute
         Compatibility. You may need to rewrite this code.
         */
         (deviceProp.get_major_version() == 7 &&
    ...
    /*
    DPCT1005:111: The SYCL device version is different from CUDA Compute
    Compatibility. You may need to rewrite this code.
    */
...
std::unique_ptr<Network> MakeCudnnNetworkAuto(
  ...
  /*
  DPCT1005:112: The SYCL device version is different from CUDA Compute
  Compatibility. You may need to rewrite this code.
  */
  if (deviceProp.get_major_version() >= 7 ||
      /*
      DPCT1005:113: The SYCL device version is different from CUDA Compute
      Compatibility. You may need to rewrite this code.
      */
      (deviceProp.get_major_version() == 6 &&
       deviceProp.get_minor_version() != 1) ||
      /*
      DPCT1005:114: The SYCL device version is different from CUDA Compute
      Compatibility. You may need to rewrite this code.
      */
      (deviceProp.get_major_version() == 5 &&
       deviceProp.get_minor_version() == 3)) {
    CERR << "Switching to [cudnn-fp16]...";
    return MakeCudnnNetwork<sycl::half>(weights, options);
  }
