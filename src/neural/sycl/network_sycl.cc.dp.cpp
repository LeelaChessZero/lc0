...
template <typename DataType> class SyclNetwork;
...
template <typename DataType> class SyclNetworkComputation : public NetworkComputation {
 public:
  CudaNetworkComputation(SyclNetwork<DataType>* network, bool wdl,
                         bool moves_left);
...
class SyclNetwork : public Network {
 public:
  ...
  void forwardEval(InputsOutputs* io, int batchSize) {
    ...
    if (fp16) {
      expandPlanes_Fp16_NCHW((sycl::half*)(tensor_mem[0]), ipDataMasks, ipDataValues, batchSize * kInputPlanes, sycl_queue_);
    } else {
    ...
    if (attn_policy_) {
      ...
      if (fp16) {
        network_[l++]->Eval(batchSize, tensor_mem[1], tensor_mem[0], nullptr,
                            scratch_mem, scratch_size_);  // policy map layer
        copyTypeConverted(opPol, (sycl::half*)(tensor_mem[1]),
                          batchSize * kNumOutputPolicy, sycl_queue_);  // POLICY output
      } else {
      ...
      if (fp16) {
        network_[l++]->Eval(batchSize, tensor_mem[1], tensor_mem[0], nullptr,
                            scratch_mem, scratch_size_);  // pol FC

        copyTypeConverted(opPol, (sycl::half*)(tensor_mem[1]),
                          batchSize * kNumOutputPolicy, sycl_queue_);  // POLICY
      } else {
        network_[l++]->Eval(batchSize, (DataType*)opPol, tensor_mem[0], nullptr,
    ...
    if (wdl_) {
      if (fp16) {
        ...
        copyTypeConverted(opVal, (sycl::half*)(tensor_mem[0]), 3 * batchSize, sycl_queue_);  // VALUE
      } else {
      ...
      if (fp16) {
        ...
        network_[l++]->Eval(batchSize, tensor_mem[0], tensor_mem[1], nullptr,
                            scratch_mem, scratch_size_);  // value FC2
        copyTypeConverted(opVal, (sycl::half*)(tensor_mem[0]), batchSize, sycl_queue_);  // VALUE
      } else {
        network_[l++]->Eval(batchSize, (DataType*)io->op_value_mem_shared_, tensor_mem[1], nullptr,
                          scratch_mem, scratch_size_);  // value FC2    // VALUE
      }
    ...
    if (moves_left_) {
      ...
      if (fp16) {
        ...
        copyTypeConverted(opMov, (sycl::half*)(tensor_mem[0]), batchSize, sycl_queue_);
      } else {
...
REGISTER_NETWORK("sycl", MakeSyclNetwork<float>, 135)
