...
class Evaluator {
 public:
  ...
  // Run for each tree.
  virtual void Gather(z9mcts::NodeTree* tree) = 0;
  // Run once between Gather and Move.
  virtual void Run() = 0;
  // Run for each tree in the same order as Gather.
  virtual void MakeBestMove(z9mcts::NodeTree* tree) = 0;
};
...
class MultiSelfPlayGames {
 public:
  ...
  std::vector<Move> GetMoves(int index) const {
    ...
    for (z9mcts::Node* node = trees_[index]->GetCurrentHead();
 ...
 private:
  ...
  // tree_[0] == tree_[1].
  std::vector<std::shared_ptr<z9mcts::NodeTree>> trees_;
