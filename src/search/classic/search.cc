...
void Search::MaybeTriggerStop(const IterationStats& stats,
  ...
  if (!stop_.load(std::memory_order_acquire)) {
    const int kGarbageCollectionIntervalMs = 1000;
    if (stopper_->ShouldStop(stats, hints)) {
      FireStopInternal();
    } else if (stats.time_since_movestart - gc_started_time_ >
        kGarbageCollectionIntervalMs) {
      NodeGarbageCollector::Instance().Start();
      gc_started_time_ = stats.time_since_movestart;
    }
  }
